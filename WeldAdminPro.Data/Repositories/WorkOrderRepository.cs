...
IX_WorkOrders_ProjectId
ON WorkOrders(ProjectId);
...
CREATE INDEX IF NOT EXISTS
IX_WorkOrders_DueDate
ON WorkOrders(DueDate);
";
        ...
        WHERE name='{column}'";
            ...
            AddColumn("ScheduledStart", "TEXT");
            AddColumn("WorkCenter", "TEXT");
            AddColumn("BlockReason", "INTEGER");
            AddColumn("BlockMessage", "TEXT");
...
WHERE Id = @Id";
		...
		public IEnumerable<WorkOrder> GetByProject(Guid projectId)
		{
			...
			using var cmd = connection.CreateCommand();
            cmd.CommandText = @" SELECT * FROM WorkOrders WHERE ProjectId = @ProjectId ORDER BY CreatedOn DESC;";
...
WHERE Key='NextWorkOrderNumber'";
        ...
        public void Add(WorkOrder workOrder)
        {
            ...
            if (!string.IsNullOrWhiteSpace(
                ...
                &&
                Exists(workOrder.WorkOrderNumber))
            {
                throw new InvalidOperationException(
                    $"Work Order '{workOrder.WorkOrderNumber}' already exists.");
            }
...
(
    ...
    @CompletedOn
            ...
            cmd.Parameters.AddWithValue(
                "@Status",
                (int)workOrder.Status);

            cmd.Parameters.AddWithValue(
                "@CreatedOn",
                workOrder.CreatedOn.ToString("O"));
