"""
WeldAdmin Pro - Import & Records (v6 Bulk + Integrated OCR + Drag & Drop)
-------------------------------------------------------------------------
New in this build:
- **Drag & Drop** support (Windows Explorer / Finder) for:
  - Import tab (drop a PDF/image to load and import)
  - Bulk Import dialog (drop a **folder** to set the scan root, or drop files to queue them)
- Still includes: Pattern-safe OCR (PyMuPDF preferred), Bulk Import with progress,
  Edit dialog, Search/Filter, Export, Delete, Auto-DB creation.

Dependencies (optional but recommended):
    pip install pymupdf
    pip install tkinterdnd2   <- enables drag & drop

Run:
    python import_wps_manager_v6_bulk_integrated_dnd.py
"""
...
# Root/Toplevel class selection for DnD
BaseTk = TkinterDnD.Tk if HAVE_DND else tk.Tk  # type: ignore[attr-defined]
BaseToplevel = TkinterDnD.Toplevel if HAVE_DND else tk.Toplevel  # type: ignore[attr-defined]
...
APP_TITLE = "WeldAdmin Pro - Import & Records (v6 Bulk + DnD)"
...
class ImportTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        # Enable DnD on the entry (drop a file to set & auto-import)
        if HAVE_DND:
            try:
        ...
        tip = "Tip: Drop a PDF/image onto the path box to import." if HAVE_DND else "Tip: Install 'tkinterdnd2' for drag-and-drop (pip install tkinterdnd2)."
        ttk.Label(footer, text=tip).pack(side="left")

    def _on_drop_file(self, event):
        files = _split_dnd_paths(self.entry_path, event.data)
        # pick first file with allowed extension
        for fp in files:
...
class BulkImportDialog(BaseToplevel):
    def __init__(self, parent, on_done=None):
        ...
        # DnD: drop a folder onto the folder entry
        if HAVE_DND:
            try:
        ...
        # DnD: drop files onto the list to queue them
        if HAVE_DND:
            try:
    ...
    def _on_drop_folder(self, event):
        ...
        # choose first that is a directory
        for pth in items:
