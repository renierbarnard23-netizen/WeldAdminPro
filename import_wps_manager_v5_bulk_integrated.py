"""
WeldAdmin Pro - Import & Records (v5 Bulk, OCR integrated & pattern-safe)
-----------------------------------------------------------------------
What's new in this build:
- Integrates the pattern-safe OCR directly into this file (no separate ocr.py needed)
- Prefers PyMuPDF rendering to avoid poppler "PatternX invalid float" warnings
- Falls back to pdf2image/pdftocairo with grayscale attempts
- Bulk Import dialog + Edit, Search/Filter, Export, Delete all retained

Run:
    python import_wps_manager_v5_bulk_integrated.py
"""
...
try:
    # If you already have parser_weldadmin.py, we'll use it
...
if not HAVE_EXT_PARSER:
    # Minimal generic parser (fallback)
    FIELD_PATTERNS = [
    ...
    def parse_weldtrace_layout(text: str) -> Dict[str, str]:
        # Light heuristics for WeldTrace-like layouts
        data: Dict[str, str] = {}
...
def extract_and_parse(path: str, tesseract_path: str = "", poppler_path: str = "") -> Dict[str, str]:
    ...
    # Parse (ext parser if available; else fallback)
...
APP_TITLE = "WeldAdmin Pro - Import & Records (v5 Bulk, OCR integrated)"
...
# ---------------- GUI: Records tab (with Bulk Import) ----------------
